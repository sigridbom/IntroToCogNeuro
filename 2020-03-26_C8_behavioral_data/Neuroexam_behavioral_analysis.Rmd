...
#writing to xlsx - kan ikke få det her til at virke, kan i?
...
fmla <- as.formula(paste("rt ~ ", paste(names(WordFace_FA[,37]), collapse= "+"),paste('+ img + (no|ID) + (1|word) + (1|delay_frames_before) + (1|time) + (1|session)')))

m3 <- glmer(fmla , data=WordFace_FA,family=Gamma(link='identity'))

WordFace_FA$img <- relevel(WordFace_FA$img, 'image_stim_n.png')


m4 <- glmer(rt ~ WordFace_FA$Unpleasant + WordFace_FA$img + (WordFace_FA$no|WordFace_FA$ID) + (1|WordFace_FA$word) + (1|WordFace_FA$delay_frames_before) + (1|WordFace_FA$time) + (1|WordFace_FA$session), data = WordFace_FA, family = Gamma(link = 'identity'))

summary(m4)

m5 <- glmer(rt ~ WordFace_FA$word_label + WordFace_FA$img + (WordFace_FA$no|WordFace_FA$ID), data = WordFace_FA, family = Gamma(link = 'identity'))
...
#find number of words used in experiment
length(unique(WordFace_FA$word))
#360

# find mean rt for words used
mean_rt <- WordFace_FA %>% group_by(word) %>% summarise(mean(rt))

#merge with paradigm scores
new_df <- merge(mean_rt, paradigm, by = 'word')

#rename column
colnames(new_df)[2] <- 'mean_rt'

as.matrix(new_df)

#make correlation df
new_df_cor <- new_df[,2:96]

# find problem columns
lapply(new_df_cor, class)

#remove those columns
columns_to_remove <- c("word", "IMG", "...81", "Type", "Super Category", "Category", "Kmeans28 Category")
new_df_cor[columns_to_remove] <- NULL

#make correlation test
cor(new_df_cor, method = 'spearman')

pacman::p_load(psych)
corr.test(new_df[,2:96], method = 'spearman')


test <- merge(new_df, binderfa, by='word')
...
pacman::p_load(ggplot2, ggpubr, plyr)

# make a plot to visualise the data

img_plot <- subset(WordFace_FA$img, WordFace_FA$rt)

WordFace_FA_group <- WordFace_FA %>% group_by(img)

WordFace_FA_group$img <- revalue(WordFace_FA_group$img, c('image_stim_n.png' = 'negative_faces', 'image_stim_p.png' = 'positive_faces'))
  
#revalue(x, c("beta"="two", "gamma"="three"))

ggline(WordFace_FA_group, x = "img", y = "rt", col = 'img', add = c("mean_se", "dodge"), palette = "jco")


ggplot(WordFace_FA_group, aes(img, rt)) + geom_bar()

#plot for valence of words

WordFace_FA$Unpleasant_cat <- ifelse(WordFace_FA$Unpleasant > 0, 'negative_word', 'positive_word')

WordFace_FA_group_cat <- WordFace_FA %>% group_by(Unpleasant_cat)

ggline(WordFace_FA_group_cat, x = "Unpleasant_cat", y = "rt", col = 'Unpleasant_cat', add = c("mean_se", "dodge"), palette = "jco")

ggplot(WordFace_FA, aes(Unpleasant_cat, rt)) + geom_point() + geom_smooth()
